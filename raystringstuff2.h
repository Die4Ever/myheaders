...
vector<string> SuperSplit_R(const char *source, const char *format);
#else
...
vector<string> SuperSplit_R(const char *source, const char *format)
{
	...
	if(formatting.size()==0) return res;
	if(formatting.back().length()==0) {
		formatting.pop_back();
		wildend=true;
	}
	...
	for(;source<sourceend && source;it++)
	{
		...
		if(/*i<i2 && */s.length()==0)
		{
			if(strstr(source, formatting[i].c_str())==NULL /*|| i<i2*/) {//% should be able to be 0 length right?
				res.push_back(source);
				break;
			}
			//s=source+formatting[i].length();
			source+=formatting[i].length();
			if (strstr(source, formatting[i2].c_str()) == NULL) {
				res.push_back(source);
				break;
			}
			else res.push_back(string());
		} else if(i>i2 && wildend==false) {
			/*if(strstr(source,formatting[i].c_str()))*/ source+=formatting[i].length();
			res.push_back(source);
			break;
		} else {
			source = strstr(source, formatting[i].c_str()) + formatting[i].length() + s.length();
...
vector<string> Old_SuperSplit_R(const char *source, const char *format, bool trim=true)
{
	const char *sourceend = source + strlen(source);
	vector<string> res;
	vector<string> formatting;
...
vector<string> Old_SuperSplit_R2(const char *source, const char *format)
{
	const char *sourceend = source + strlen(source);
	vector<string> res;
	vector<string> formatting;
